        ...
        for block in page.get('blocks', []):
            ...
            if block.get('semantic_type') == 'heading_1':
                current = {
                    ...
                    'page_end': block['page'],
                    'blocks': [compact_block(block)],
                }
                sections.append(current)
            elif current is None:
                preface.append(compact_block(block))
            else:
                current['blocks'].append(compact_block(block))
                current['page_end'] = block['page']
    if preface:
        sections.insert(0, {
            'id': 'preface', 'title': 'Front matter',
            'page_start': preface[0]['page'], 'page_end': preface[-1]['page'], 'blocks': preface
        })
    for section in sections:
        section['text'] = '\n'.join(block['text'] for block in section['blocks'])
        section['headings'] = [block for block in section['blocks'] if block.get('semantic_type', '').startswith('heading_')]
        section['paragraph_count'] = sum(1 for block in section['blocks'] if block.get('semantic_type') == 'paragraph')
    return sections
...
def derive_candidates(evidence: dict[str, Any]):
    ...
    for page in evidence.get('pages', []):
        for block in page.get('blocks', []):
            if block.get('semantic_type') == 'quote_candidate':
                quotes.append(compact_block(block))
        for item in page.get('derived', []):
            ...
            if typ == 'number':
                ...
                images.append({
                    **{k: item.get(k) for k in ('id', 'page', 'bbox', 'area_ratio', 'asset_id', 'caption') if item.get(k) is not None},
                    'asset': {k: asset.get(k) for k in ('id', 'sha256', 'dhash', 'width', 'height', 'extension', 'file') if asset.get(k) is not None},
                })
...
def prepare(evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        'version': VERSION,
        'kind': 'storyscro_planner_input',
        'source': {
            'file': evidence['document']['source_file'],
            'sha256': evidence['document']['sha256'],
            'page_count': evidence['document']['page_count'],
            'language': evidence['document'].get('language', 'und'),
        },
        'document_candidates': {
            'preferred_title': title,
            'title_candidates': evidence.get('title_candidates', []),
            'sections': [{k: section[k] for k in ('id', 'title', 'page_start', 'page_end', 'text', 'headings', 'paragraph_count')} for section in sections],
        },
        'evidence_candidates': {
            'numbers': numbers,
            'tables': tables,
            'images': images,
            'vector_visuals': vectors,
            'quotes': quotes,
            'repeated_margin_elements': evidence.get('repeated_margin_elements', []),
        },
        ...
        'constraints': {
            ...
            'allowed_statuses': ['VERIFIED', 'INFERENCE', 'HYPOTHESIS', 'TO_CONFIRM', 'NOT_FOUND'],
            'default_fidelity': 'adaptive',
...
def main():
    ...
    parser.add_argument('--out', type=Path, default=Path('planner-input.json'))
    args = parser.parse_args()
    output = prepare(load(args.evidence))
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps(output, ensure_ascii=False, indent=2), encoding='utf-8')
    print(json.dumps({
        'out': str(args.out),
        'sections': len(output['document_candidates']['sections']),
        'numbers': len(output['evidence_candidates']['numbers']),
        'tables': len(output['evidence_candidates']['tables']),
        'images': len(output['evidence_candidates']['images']),
        'vectors': len(output['evidence_candidates']['vector_visuals']),
